...
This section presents a controlled empirical study of the proposed modular reasoning framework.  
Instead of combining multiple independent models, we adopt a *single* shared backbone (Qwen3-0.6B) equipped with several lightweight expert heads, each trained on a disjoint numerical sub-domain.  
This design allows us to directly observe how modular structure affects:

1. **Local specialization** – experts trained on specific value ranges.
2. **Global recomposition** – a router that integrates experts into a full-range predictor.
3. **Structural robustness** – stability under expert addition, removal, or replacement.
4. **Computational efficiency** – parameter and runtime overhead relative to non-modular baselines.

To make the analysis transparent and reproducible, we focus on arithmetic reasoning with explicit numerical ranges and closed-form ground truth.

## 4.1 Experimental Overview

All models receive natural-language arithmetic queries and predict the corresponding integer result as a classification label.  
A typical input–output pair is of the form:

- Input: “What is 37 plus 12?”
- Output: the integer \(49\).

The numerical output space is restricted to the interval \([2, 100]\) and partitioned into five non-overlapping sub-ranges:
\[
R_1 = [2, 20],\quad
R_2 = [21, 40],\quad
R_3 = [41, 60],\quad
...
Each sub-range \(R_i\) is assigned a dedicated expert head, while all experts share a common Qwen3-0.6B backbone encoder.  
The backbone is largely frozen, with only the top two transformer layers kept trainable to adapt high-level numerical reasoning, which ensures that:

- low-level linguistic representations remain shared, and  
- high-level arithmetic patterns can specialize toward different ranges.

The training procedure is **two-stage**:

1. **Expert training.**  
   For each range \(R_i\), we train the corresponding expert head (and the shared top layers) on samples whose true results lie in \(R_i\).  
   This yields locally specialized predictors that operate on a common representation space.

2. **Router training.**  
   Once the experts have converged, we freeze them and train a lightweight router on the full dataset.  
   The router observes the shared backbone representation and learns to:
   - predict the appropriate numerical sub-range, and  
   - combine expert outputs via residual composition (detailed in Section 4.4).

This setup mirrors practical modular systems in which experts may come from heterogeneous sources (tasks, domains, or devices), while the router is the only component that must be retrained to integrate them.  
By using a simple but fully controllable arithmetic environment, the experiments isolate the effects of modular structure and provide a clean testbed consistent with Knowledge-Based Systems evaluation practice.
...
Both tasks share the same synthetic dataset configuration:

- **20,000 training samples**
- **2,500 validation samples**
- **2,500 test samples**

The **8 : 1 : 1** split ensures each numerical range is proportionally sampled, and the full label space $[2, 100]$ is partitioned into five disjoint ranges (see Task 1) for dedicated expert heads.
...
Task 2 reuses the five ranges defined in Task 1, which allows us to test the **transferability** of experts trained on addition when the underlying operator structure changes.

The dataset split is the same 8 : 1 : 1 configuration described above, so no additional sampling adjustments are required.
