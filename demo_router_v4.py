...
demo_router_qwen.py
-------------------
使用 Qwen3-0.6B 作为共享编码器的 Modular + Router 加法 QA Demo：
...
任务：
  文本加法：
    ...
    A: "20"
  1 <= a,b <= 100; 2 <= a+b <= 200
  label 空间：["2", "3", ..., "200"] 共 199 类
  按 sum 分 5 个区间：
    [2,40], [41,80], [81,120], [121,160], [161,200]

模型结构：
  - encoder: 冻结的 Qwen3-0.6B (AutoModel)
  - base_head: 线性分类头
  - experts: 5 个残差专家 (small heads)
  - router: 线性门控，softmax 得到 α_i(x)

三种模式对比：
  1) Base-only
  2) Base + AllExperts (固定 alpha=0.1)
  3) Base + Router (learned gating，缩放 0.1)
"""
...
ENCODER_UNFREEZE_LAST_LAYERS = 4  # Base 阶段可解冻 encoder 的最后几层
...
def build_addition_dataset(num_samples=20000, seed=42):
    ...
    def sum_to_domain(s):
        ...
        if 81 <= s <= 120:
            return "range_81_120"
        if 121 <= s <= 160:
            return "range_121_160"
        if 161 <= s <= 200:
            return "range_161_200"
        return None
...
class QwenEncoder(nn.Module):
    """
    封装 Qwen3-0.6B 的 encoder，输出 pooled 表示 h(x) ∈ R^H
    """
    ...
    def forward(self, input_ids, attention_mask):
        ...
        h_pooled = h_sum / lengths          # (B, H)
        return h_pooled


def _collect_encoder_layers(auto_model: nn.Module) -> List[nn.Module]:
    """
    Collect transformer layers inside the AutoModel so we can unfreeze selectively.
    """
    if hasattr(auto_model, "encoder") and hasattr(auto_model.encoder, "layers"):
        return list(auto_model.encoder.layers)
    if hasattr(auto_model, "transformer") and hasattr(auto_model.transformer, "encoder"):
        enc = auto_model.transformer.encoder
        if hasattr(enc, "layers"):
            return list(enc.layers)
    if hasattr(auto_model, "layers"):
        return list(auto_model.layers)
    return []


def set_encoder_trainable_layers(encoder: QwenEncoder, num_layers: int):
    """
    Freeze all encoder layers and unfreeze the last `num_layers` of them.
    """
    for p in encoder.parameters():
        p.requires_grad = False
    if num_layers <= 0:
        return
    layers = _collect_encoder_layers(encoder.model)
    if not layers:
        return
...
class ModularAddModelWithRouter(nn.Module):
    def __init__(self, encoder: QwenEncoder, num_classes,
                 num_experts=5):
        super().__init__()
...
def train_base(model, dataloader, epochs=6, lr=1e-4):
    """
    训练 base_head（分类头），Qwen encoder 冻结不训练
    """
    model.train()

    set_encoder_trainable_layers(model.encoder, ENCODER_UNFREEZE_LAST_LAYERS)
    ...
    for ep in range(1, epochs + 1):
        ...
        print("=> [Base] epoch {:3d} done | loss={:.4f}, acc={:.4f}".format(
            ep, total_loss / total_count, total_correct / total_count
        ))

def train_expert(model, dataloader, expert_idx, epochs=5, lr=5e-4):
    '''Train a single residual expert while keeping encoder/base/router frozen.'''
    model.train()
    ...
    for ep in range(1, epochs + 1):
        ...
        pbar = tqdm(dataloader,
                    desc="[Expert {}] epoch {}/{}".format(expert_idx, ep, epochs),
                    ncols=100)
        for batch in pbar:
            input_ids = batch["input_ids"].to(DEVICE)
            attention_mask = batch["attention_mask"].to(DEVICE)
        ...
        print(
            ">= [Expert {}] epoch {:3d} done | loss={:.4f}, acc={:.4f}".format(
...
def train_router_domain(model, dataloader, domain_to_id, epochs=5, lr=5e-4):
    '''Freeze encoder/base/expert and train the router via domain supervision.'''
    model.train()
    for p in model.encoder.parameters():
        p.requires_grad = False
    for p in model.base_head.parameters():
    ...
    for ep in range(1, epochs + 1):
        ...
        pbar = tqdm(dataloader,
                    desc="[Router-domain] epoch {}/{}".format(ep, epochs),
                    ncols=100)
        for batch in pbar:
            input_ids = batch["input_ids"].to(DEVICE)
            attention_mask = batch["attention_mask"].to(DEVICE)
        ...
        print("=> [Router-domain] epoch {:3d} done | loss={:.4f}, acc={:.4f}".format(
            ep, total_loss / total_count, total_correct / total_count
        ))
...
def evaluate_range_with_mode(model, loaders_per_range, mode, tag, num_experts=5):
    ...
    ranges = ("2-40", "41-80", "81-120", "121-160", "161-200")
...
def main():
    ...
    print("Label vocab size:", len(label_vocab))  # 应该是 199
    ...
    # tokenizer & Qwen encoder
    QWEN_PATH = r"E:\dev\test2\Qwen3-0.6B"  # 改成你的本地路径
    ...
    # 构建 Qwen encoder + Modular 模型
    encoder = QwenEncoder(QWEN_PATH).to(DEVICE)
