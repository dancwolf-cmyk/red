...
SAVE_DIR = "checkpoints_v13_1"
LOG_FILE = Path(SAVE_DIR) / "demo_v13_1.log"
MODEL_PATH = r"e:/dev/lunwen/Qwen3-0.6B"

STAGE_STATUS_FILE = Path(SAVE_DIR) / "stage_status_v13_1.json"
SAMPLES_CACHE_FILE = Path(SAVE_DIR) / "demo_v13_1_samples_mixed.json"
BALANCED_TRAIN_CACHE_FILE = Path(SAVE_DIR) / "demo_v13_1_train_balanced.json"
BALANCED_VAL_CACHE_FILE = Path(SAVE_DIR) / "demo_v13_1_val_balanced.json"
BALANCED_TEST_CACHE_FILE = Path(SAVE_DIR) / "demo_v13_1_test_balanced.json"
...
def setup_logger():
    ...
    logger = logging.getLogger("demo_v13_1")
    if logger.handlers:
        return logger
...
def save_checkpoint(model, stage, epoch=None):
    ...
    path = Path(SAVE_DIR) / "demo_v13_1_{}{}.pt".format(stage, suffix)
...
def get_best_checkpoint(stage_name):
    matches = sorted(Path(SAVE_DIR).glob("demo_v13_1_{}_best*.pt".format(stage_name)))
    if not matches:
        return None
...
def train_base(model, loader, val_loader, epochs=6, lr=5e-5, virtual_batch_size=VIRTUAL_BATCH_SIZE):
    ...
    loss, acc, val_loss, val_acc = 0.0, 0.0, float("inf"), 0.0
...
def train_moe_router(model: ModularAddModelWithMoE,
    ...
    for epoch in range(1, epochs + 1):
        ...
        if avg_loss < best_loss:
            ...
            best_loss = avg_loss
            best_epoch = epoch
            best_path = Path(SAVE_DIR) / "demo_v13_1_moe_best_ep{}.pt".format(epoch)
...
def main():
    ...
    if not is_stage_completed(base_stage):
        ...
        for epoch in range(1, MAX_EPOCHS_BASE + 1):
            train_loss, train_acc, val_loss, val_acc = train_base(
            ...
            if val_acc > best_acc:
                ...
                patience_cnt += 1
                if patience_cnt >= PATIENCE:
                    logger.info("=> Early stopping {} at epoch {} by val_acc (patience={})".format(
    ...
    for ei in range(5):
        ...
        if not is_stage_completed(expert_stage):
            ...
            for epoch in range(1, MAX_EPOCHS_EXPERT + 1):
                train_loss, train_acc, val_loss, val_acc = train_expert(
