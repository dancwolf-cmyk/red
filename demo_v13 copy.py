...
class QwenEncoderFrozen(nn.Module):
    def __init__(self, model_path, max_len=64, trainable_last_n_layers=1):
        ...
        try:
            ...
            # 某些老版本 transformers / Qwen 代码不支持 torch_dtype 等参数，降级处理
        ...
        # 如果在 CUDA 上，用 AMP 的 dtype 统一一遍（防止混 dtype 太乱）
        if torch.cuda.is_available():
            try:
...
class ModularAddModelWithMoE(nn.Module):
    """
    经典 MoE gating：
      - encoder 输出 h -> router 得到各 expert 的权重
      - 每个 expert 是与 RED 相同结构的 MLP：Linear(H -> 4H) + GELU + Linear(4H -> num_classes)
      - 直接将 experts 的 logits 按 router softmax 加权求和（不包含 base residual）
    """
    def __init__(self, encoder, num_classes, num_experts=5):
        super().__init__()
        self.encoder = encoder
        ...
        # 保留一个 base_head（即便 forward 不用，也方便从 base 阶段 checkpoint 里继承，
        # 同时结构上和 RED 模型保持一定对齐）
        self.base_head = nn.Linear(H, num_classes)

        # expert 结构与 RED 的 ModularAddModelWithRouter 完全一致
        hidden_exp = 4 * H
        self.experts = nn.ModuleList(
            [
                nn.Sequential(
        ...
        # router：从 encoder 输出到 num_experts 的 gating
        self.router = nn.Linear(H, num_experts)
        self.num_experts = num_experts
...
def main():
    ...
    expert0_encoder = QwenEncoderFrozen(MODEL_PATH, trainable_last_n_layers=1).to(DEVICE)
